# app.py — 3-2 공급량상세 대시보드 (레포 파일/업로드 + 사이드바 UI)
# - 레포 루트의 xlsx/csv를 선택하거나 업로드 파일 사용
# - 엑셀은 "3-2 공급량상세" 시트를 우선 선택(있으면 자동 기본)
# - 컬럼 매핑 UI → 정규화 → 요약표/그래프/다운로드
...
st.caption("연도·시나리오·용도별 요약표와 동적 그래프 · 레포 파일 또는 업로드 데이터 사용")
...
src = sb.radio("데이터 소스 선택", source_options, index=0, horizontal=False, key="src_radio")
...
def pick_repo_file():
    ...
    if not files:
        sb.warning("레포에 xlsx/csv 파일이 없습니다. 업로드를 선택하세요.")
        return None, None

    sb.subheader("📁 레포 파일 선택")
    labels = [p.name for _, p in files]
    idx = sb.selectbox("파일", options=list(range(len(labels))), format_func=lambda i: labels[i], key="repo_file_idx")
    kind, path = files[idx]
    ...
    if kind == "xlsx":
        ...
        sn = sb.selectbox("시트", options=names, index=default_idx, key="repo_sheet_name")
        return sheets[sn], sn
    else:
...
# 분기별 로딩
if src == "레포에 있는 파일 사용":
    raw, sheet_name = pick_repo_file()

elif src == "엑셀 업로드(.xlsx)":
    sb.subheader("📤 엑셀 파일 업로드")
    up = sb.file_uploader("Drag & drop 또는 클릭해서 선택", type=["xlsx"], key="upload_xlsx")
    if up:
        sheets = load_excel(up.getvalue())
        names = list(sheets.keys())
        sb.subheader("🗂 시트 선택")
        idx = names.index("3-2 공급량상세") if "3-2 공급량상세" in names else 0
        sheet_name = sb.selectbox("시트", options=names, index=idx, key="upload_sheet_name")
        raw = sheets[sheet_name]

elif src == "CSV 업로드(.csv)":
    sb.subheader("📤 CSV 업로드")
    upc = sb.file_uploader("Drag & drop 또는 클릭해서 선택", type=["csv"], key="upload_csv")
    if upc:
        raw = load_csv(upc.getvalue())

if raw is None:
    st.info("좌측에서 **데이터 소스 선택 → 파일/시트 선택**을 완료하면 미리보기가 나타납니다.")
    st.stop()
