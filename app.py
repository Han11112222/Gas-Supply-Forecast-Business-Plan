...
def main() -> None:
    ...
    if page == "실적 분석":
        ...
        years = st.multiselect(
            ...
            default=default_years_2020_ or nonzero_years,
            key="trend_years",
        )
    ...
    # ---------------- 계획대비 분석 ----------------
        ...
        with col_toggle2:
            include_prev_for_month = st.toggle("(Y-1) 포함", value=True, key="toggle_month_prev")

        prev_month_year = year_month - 1
        has_prev_month = prev_month_year in year_options

        month_df = get_group_df(df_all, month_group)
        month_df = month_df[month_df["연"].isin([year_month, prev_month_year] if has_prev_month else [year_month])]

        # 기간 필터
        if period == "상반기(1~6월)":
            month_df = month_df[month_df["월"].between(1, 6)]
        elif period == "하반기(7~12월)":
            month_df = month_df[month_df["월"].between(7, 12)]

        # 현재 연도/이전 연도 분리
        cur_df = month_df[month_df["연"] == year_month].sort_values("월")
        prev_df = month_df[month_df["연"] == prev_month_year].sort_values("월") if has_prev_month else pd.DataFrame()

        # 증감 계산 (현재 연도 기준)
        diff = cur_df["실적"].values - cur_df["계획"].values
        diff_x = cur_df["월"].values
        ...
        fig2.add_trace(
            go.Bar(
                x=cur_df["월"],
                y=cur_df["실적"],
                name=f"{year_month}년 실적",
                marker_color=COLOR_ACT,
            )
        )

        if include_prev_for_month and has_prev_month and not prev_df.empty:
            fig2.add_trace(
                go.Bar(
                    x=prev_df["월"],
                    y=prev_df["실적"],
                    name=f"{prev_month_year}년 실적",
                    marker_color=COLOR_PREV,
                )
            )
        ...
        # 월별 요약표 (그래프 하단)
        summary_month = cur_df.copy()
        summary_month["차이(실적-계획)"] = diff
        for col in ["계획", "실적", "차이(실적-계획)"]:
            summary_month[col] = summary_month[col].map(format_number)
        st.markdown("#### 📋 월별 계획·실적·증감 요약")
        st.dataframe(
            summary_month[["연", "월", "계획", "실적", "차이(실적-계획)"]],
            use_container_width=True,
            hide_index=True,
        )
    ...
    # ---------------- 기간별 누적 실적 ----------------
        ...
        df = df_all[df_all["연"].isin(years_stack)].copy()

        if period == "상반기(1~6월)":
            df = df[df["월"].between(1, 6)]
        elif period == "하반기(7~12월)":
            df = df[df["월"].between(7, 12)]

        agg = (
            df.groupby(["연", "그룹"], as_index=False)[["실적"]]
            .sum()
        ...
        # 총량, 가정용 합계 계산
        agg["총량"] = agg.sum(axis=1)
        if "가정용" in agg.columns:
            agg["가정용_only"] = agg["가정용"]
        else:
            agg["가정용_only"] = 0.0

        years_sorted = sorted(agg.index)
        group_cols = [c for c in agg.columns if c not in ("총량", "가정용_only")]
        ...
        fig.add_trace(
            go.Scatter(
                x=years_sorted,
                y=agg.loc[years_sorted, "가정용_only"],
                name="가정용",
                mode="lines+markers",
                line=dict(color=COLOR_ACT, width=3, dash="dot"),
                yaxis="y2",
            )
        )
        fig.add_trace(
            go.Scatter(
                x=years_sorted,
                y=agg.loc[years_sorted, "총량"],
                name="합계",
                mode="lines+markers",
                line=dict(color=COLOR_DIFF, width=3),
                yaxis="y2",
            )
        )
    ...
    # ---------------- 연도별 총 실적 ----------------
        ...
        df = df_all.copy()
        agg = df.groupby("연", as_index=False)[["실적"]].sum()
        agg = agg[agg["실적"] > 0].sort_values("연")
        ...
        fig.update_layout(
            xaxis=dict(title="연도"),
            yaxis=dict(title=f"연간 공급량 ({unit_str})"),
            hovermode="x",
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        display = agg.copy()
        display["실적"] = display["실적"].map(format_number)
